...
`isProtectedPage` is `true` when the URL path contains `/pages/`. `isAdminRoute` is `true` when the path matches the pattern `/admin|/settings|user-management/` (case-insensitive). The practical effect is:
...
Switches the account's `status` between `'active'` and `'inactive'`. **Admin only**. An inactive account cannot log in — `authenticateUser` will fail for accounts with `status === 'inactive'` as they will not pass credential validation.
