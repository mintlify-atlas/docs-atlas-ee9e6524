---
title: "Roles and Permissions in Amvel Warehouse"
sidebarTitle: "Roles & Permissions"
description: "How the two user roles — Admin and Employee — differ in Amvel Warehouse, and how route guards enforce access at page load."
---
...
2. **Admin-only route check** — If the current URL path matches the pattern `/\/admin|\/settings|user-management/i`, the function additionally verifies that `AuthService.getCurrentUser().role === 'admin'`. If the logged-in user is not an admin, the browser is redirected to `dashboard.html`.

3. **Nav link visibility** — Elements with the CSS class `admin-only-nav` (such as the **User Management** dropdown link) are shown only when `isAdminUser()` returns `true`. Employee sessions leave those elements hidden.
...
<Note>
  The `isAdminRoute` regex used in `guardPageAccess` is `/\/admin|\/settings|user-management/i`. Any page whose URL path contains one of these patterns — `/admin`, `/settings`, or `user-management` — is treated as an admin-only route and requires the `admin` role to access.
</Note>
