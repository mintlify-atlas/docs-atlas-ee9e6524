...
description: "The Inventory page in Amvel Warehouse lists all cargo currently in the warehouse. Learn to filter by date, transaction type, and location, and export records."
---
...
| **Location**         | Warehouse rack address where the cargo is stored (e.g. `RACK A AA1A`).                               |
...
**Warehouse Location** — A free-text field with autocomplete. Begin typing a rack letter (`A`–`D`) or row code to narrow to a specific storage zone. All valid rack addresses are suggested (e.g. `RACK A AA1A`, `RACK B CB6C`).
...
| Location    | Meaning                                      |
| ----------- | -------------------------------------------- |
| `RACK A AA1A` | Rack A, Row A, Column 1                   |
| `RACK B CB6C` | Rack B, Row C, Column 6                   |
| `RACK C DF14F` | Rack C, Row F, Column 14 (end of rack)   |
| `RACK D BA3B` | Rack D, Row B, Column 3                   |
