---
title: "Dashboard: Live KPIs and Activity Report"
...
Three animated cards sit at the top of the page, each pulling its value directly from the stored shipment data.

<CardGroup cols={3}>
  <Card title="Total Shipments" icon="local_shipping">
    The all-time count of unique shipment references (grouped by HAWB or MAWB) that have ever been recorded in the warehouse. Reflects every inbound and outbound entry combined.
  </Card>
  <Card title="Cargo in Warehouse" icon="warehouse">
    The sum of remaining quantities across all active inventory records — that is, total quantity received minus total quantity released. Labelled **Current** beneath the count.
  </Card>
  <Card title="Outgoing Today" icon="box-arrow-up">
    The number of distinct shipment records that have at least one outbound entry dated today. Labelled **Shipments** beneath the count.
  </Card>
</CardGroup>
...
**Warehouse Location** — A free-text field with autocomplete suggestions covering every rack position in the warehouse (e.g. `RACK A AA1A`). Type a rack letter or row code to filter by storage location.
